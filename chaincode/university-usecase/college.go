...
type Student struct {
	...
	RoleNumber    string `json:"RoleNumber"` 
}
...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	if function == "requestAffiliation" {
		...
		return s.verifyCertificate(APIstub, args)
	}
...
// VerifyCertificate
func (s *SmartContract) verifyCertificate(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	fmt.Printf("- VerifyCetificate:\n%s\n", args[0])
	if len(args) != 2 {
		return shim.Error("Incorrect number of arguments. Expecting 2")
	}
	...
	collegeAsBytes, err := APIstub.GetState(args[0])
	//studentByName ,err := APIstub.GetState()


	if err != nil && {
		return shim.Error("Student already have certificate !!")
	} else {
		return shim.Success("Certificate verified successfully")
	}
...
func (s *SmartContract) takeAdmission(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var student = Student{StudentID: id, Name: args[0], DateOfBirth: args[1], CollegeName: args[2], AdharNumber: args[3],ProgramName: args[3]}
	student.RoleNumber = 1023
...
func (s *SmartContract) issueCertificate(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if student.CertificateID != "" {
		shim.Error("Certificate has already been assigned to Student!!")
	} else {
